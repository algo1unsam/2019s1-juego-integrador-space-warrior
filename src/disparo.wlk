...
class Disparo {
	...
	method avanzarEnTablero(disparador){
 		if (disparador == nave){	game.onTick(500, "avanzarEnTablero"+self.identity(), { => self.subirUnLugar()})}
 			else game.onTick(1000, "avanzarEnTablero"+self.identity(), { => self.bajarUnLugar()})
 	}
